...
void Preprocess::process(const livox_ros_driver::msg::CustomMsg::UniquePtr &msg, PointCloudXYZI::Ptr& pcl_out)
{
  avia_handler(msg);
  *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler(const livox_ros_driver::msg::CustomMsg::UniquePtr &msg)
{
  // 清除之前的点云缓存
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 1; i < plsize; i++)
    {
          ...
          ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        pl_full[i].curvature =
            msg->points[i].offset_time / float(1000000);         // 使用曲率作为每个激光点的时间
...
void Preprocess::helios_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.points.size();
  // printf("[ helios_handler ] Input point number: %d \n", plsize);
  if (plsize == 0) return;
  pl_surf.reserve(plsize);
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    added_pt.z = pl_orig.points[i].z;
    added_pt.intensity = pl_orig.points[i].intensity;
    added_pt.curvature = pl_orig.points[i].time_offset * time_unit_scale;

    if (i % point_filter_num == 0)
    {
      double distance = added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z;
      if (distance > blind_sqr)
      {
        pl_surf.points.push_back(added_pt);
      }
    }
  }
  // printf("[ helios_handler ] Output point number: %zu \n", pl_surf.points.size());
}
...
void Preprocess::give_feature(pcl::PointCloud<PointType>& pl, vector<orgtype>& types)
{
  ...
  for (uint i = head; i < plsize2; i++)
  {
    ...
    if (plane_type == 1) // 求得平面，并返回类型0 1 2
    {
      // 设置确定的平面点和可能的平面点
