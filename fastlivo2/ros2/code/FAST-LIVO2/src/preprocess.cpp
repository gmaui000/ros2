...
void Preprocess::process(const livox_ros_driver::msg::CustomMsg::SharedPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  avia_handler(msg);
  *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler(const livox_ros_driver::msg::CustomMsg::SharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::helios_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    added_pt.z = pl_orig.points[i].z;
    added_pt.intensity = pl_orig.points[i].intensity;
    added_pt.curvature = pl_orig.points[i].time_offset * 1.e-6f;  // 这里统一单位ms

    if (i % point_filter_num == 0 && added_pt.curvature > 0.0f)
    {
