...
LIVMapper::LIVMapper(rclcpp::Node::SharedPtr &node, std::string node_name, const rclcpp::NodeOptions & options)
    : node(std::make_shared<rclcpp::Node>(node_name, options)),
      extT(0, 0, 0),extT_I2B(0, 0, 0),
      extR(M3D::Identity()),extR_I2B(M3D::Identity())
{
  extrinT_I2B.assign(3, 0.0);
  extrinR_I2B.assign(9, 0.0);
  ...
  pcl_wait_save.reset(new PointCloudXYZRGB());                // 待保存为PCD的RGB点云
  pcl_wait_save_intensity.reset(new PointCloudXYZI());        // 待保存为PCD的强度值点云
  voxelmap_manager.reset(
    new VoxelMapManager(voxel_config, voxel_map));            // 体素地图q管理器
  vio_manager.reset(new VIOManager());                        // VIO管理器
  root_dir = ROOT_DIR;                                        // 根路径 来自CMakeLists.txt 默认为源码根目录
  initializeFiles();                                          // 初始化PCD和Colmap输出的保存路径及文件，初始化状态预测及更新对应的文件
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<int>("common.img_down", 1);
  try_declare.template operator()<int>("common.lidar_en", 1);
  try_declare.template operator()<int>("common.real_camera_init_en", 0);
  ...
  try_declare.template operator()<int>("preprocess.max_points_num", 24000);
  ...
  try_declare.template operator()<vector<double>>("extrin_calib.extrinsic_T_I2B", vector<double>{});
  try_declare.template operator()<vector<double>>("extrin_calib.extrinsic_R_I2B", vector<double>{});
  ...
  this->node->get_parameter("common.img_down", img_down);
  this->node->get_parameter("common.lidar_en", lidar_en);
  this->node->get_parameter("common.real_camera_init_en", real_camera_init_en);
  ...
  this->node->get_parameter("preprocess.max_points_num", p_pre->max_points_num);
  ...
  this->node->get_parameter("extrin_calib.extrinsic_T_I2B", extrinT_I2B);
  this->node->get_parameter("extrin_calib.extrinsic_R_I2B", extrinR_I2B);
...
void LIVMapper::initializeComponents(rclcpp::Node::SharedPtr &node) 
{
  ...
  extT_I2B << VEC_FROM_ARRAY(extrinT_I2B);
  extR_I2B << MAT_FROM_ARRAY(extrinR_I2B);
  ...
  if (!imu_en) p_imu->disable_imu();                                  // 使能IMU
  if (!gravity_est_en) p_imu->disable_gravity_est();                  // 重力估计
  if (!ba_bg_est_en) p_imu->disable_bias_est();                       // 偏差估计
  if (!exposure_estimate_en) p_imu->disable_exposure_est();           // 曝光估计
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  // VIO 更新后，发布世界系的当前点云（LVIO 模式为 RGB 点云）
  ...
  // 发布 LIO 更新后的位姿，由函数 publish_odometry 调用
  ...
  // 一个周期为 0.004 秒（即 4 毫秒，250Hz）的定时器。每当定时器触发时，会自动调用 LIVMapper::imu_prop_callback 成员函数
  ...
  static_tf_broadcaster_ = std::make_unique<tf2_ros::StaticTransformBroadcaster>(*(this->node));
...
bool LIVMapper::calcCameraInitinRealWorld()
{
  auto first_imu_time = rclcpp::Time(static_cast<int64_t>(_first_imu_time * 1e9));
  geometry_msgs::msg::TransformStamped base_to_utm;
  
  try {
    base_to_utm = tf_buffer_->lookupTransform("UTM49", "base_link", 
                                              first_imu_time,
                                              tf2::durationFromSec(1.0));
    std::cout << "TF transform found successfully" << std::endl;
  } catch(tf2::TransformException &ex) {
    RCLCPP_WARN(this->node->get_logger(), "Could not get base_link to UTM49 transform: %s", ex.what());
    std::cout << "TF transform not found, using identity transform" << std::endl;
  ...
  V3D imu_in_utm = utm_rotation * extT_I2B + V3D(
  ...
  Eigen::Quaterniond camera_init_to_utm = utm_rotation * Eigen::Quaterniond(extR_I2B);

  // 输出关键信息
  std::cout << "=== camera_init在真实世界中的坐标 ===" << std::endl;
  std::cout << "时间戳: " << _first_imu_time << std::endl;
  std::cout << "UTM49坐标系下的位置: [" 
            << imu_in_utm.x() << ", " 
            << imu_in_utm.y() << ", " 
            << imu_in_utm.z() << "]" << std::endl;
  std::cout << "旋转四元数: [" 
            << camera_init_to_utm.x() << ", "
            << camera_init_to_utm.y() << ", "
            << camera_init_to_utm.z() << ", "
            << camera_init_to_utm.w() << "]" << std::endl;
  std::cout << "===================================" << std::endl;

  geometry_msgs::msg::TransformStamped static_transform;
  static_transform.header.stamp = this->node->now();
  static_transform.header.frame_id = "UTM49";
  static_transform.child_frame_id = "camera_init";
  
  // 设置平移
  static_transform.transform.translation.x = imu_in_utm.x();
  static_transform.transform.translation.y = imu_in_utm.y();
  static_transform.transform.translation.z = imu_in_utm.z();
  
  // 设置旋转 - 使用正确的camera_init到UTM49的旋转
  static_transform.transform.rotation.x = camera_init_to_utm.x();
  static_transform.transform.rotation.y = camera_init_to_utm.y();
  static_transform.transform.rotation.z = camera_init_to_utm.z();
  static_transform.transform.rotation.w = camera_init_to_utm.w();
...
void LIVMapper::processImu() 
{
  ...
  if (real_camera_init_en && _first_imu_time+1 < 1e-6) {
    double diff = (_state.rot_end - M3D::Identity()).norm();
    if (diff > 1e-6) {
      _first_imu_time = stamp2Sec(LidarMeasures.measures.back().imu.front()->header.stamp);
      std::cout << "Setting _first_imu_time to: " << _first_imu_time << std::endl;
      calcCameraInitinRealWorld();
    }
  }

  // 记录状态，并将先验状态_state 和去畸变点云 feats_undistort 给体素地图管理器voxelmap_manager
...
{
  // 获得状态的旋转欧拉角 euler_cur 同时将上次 LIO 获得的状态输出到文件fout_pre
  ...
  // 判断 这一帧图像捕获时间与第一帧点云帧头时间差再减去-10 的绝对值是否 小于 0.3（帧计数 6/2 * 0.1 ）
...
// 首先对点云进行体素滤波，并投到体素管理器中进行状态估计
// （EKF 的更新部分），优化后根据需要保存状态到评估文件，更新体素管理器的点不确
// 定性、平面属性，将点转到世界系后进行发布和打印设置。
void LIVMapper::handleLIO() 
{  
  // 将先验旋转状态转为欧拉角  
  euler_cur = RotMtoEuler(_state.rot_end);
  // 将先验状态输出 fout_pre 到 文件 mat_pre.txt 中
  fout_pre << setw(20) << LidarMeasures.last_lio_update_time - _first_lidar_time << " " << euler_cur.transpose() * 57.3 << " "
           << _state.pos_end.transpose() << " " << _state.vel_end.transpose() << " " << _state.bias_g.transpose() << " "
           << _state.bias_a.transpose() << " " << V3D(_state.inv_expo_time, 0, 0).transpose() << endl;
  // 检查去畸变特征点云是否有点，没有点直接退出 
  ...
  downSizeFilterSurf.filter(*feats_down_body);  // feats_down_body 体素滤波后的去畸变点云
  
  double t_down = omp_get_wtime();

  // 获得各系下的体素点云并构建体素地图

  // 体素图管理器 voxelmap_manager 获得 lidar 系和世界系下的降采样点云
  ...
  // 判断地图是否初始化，第一次运行时执行 if (!lidar_map_inited)（仅执行一次，用于初始化根节点）
  if (!lidar_map_inited) 
  {
  ...
  // 状态估计中还会再计算一次点的不确定性 更新体素管理器的状态和协方差（后验）
  // _state ，以及基于后验状态获得的不确定性点的列表**_pv_list **=
  ...
  // 将状态估计后的旋转转为欧拉角 euler_cur 再转为四元数 geoQuat
  euler_cur = RotMtoEuler(_state.rot_end);
  ...
  // 声明一个世界激光雷达点云指针 并按 状态估计后的状态将去畸变的体素点云转到世界系下
  PointCloudXYZI::Ptr world_lidar(new PointCloudXYZI());
  transformLidar(_state.rot_end, _state.pos_end, feats_down_body, world_lidar);
  // 遍历世界系下的当前帧点获得更新完不确定性点的列表
  ...
  // 根据该列表更新体素地图：输入的是不确定性点列表
  voxelmap_manager->UpdateVoxelMap(voxelmap_manager->pv_list_);
  std::cout << "[ LIO ] Update Voxel Map" << std::endl;
  // 将更新后的不确定性点与 vio 管理器进行共享
  _pv_list = voxelmap_manager->pv_list_;
  
  double t4 = omp_get_wtime();

  // 判断 体素管理器的地图滑动使能（清除超过某一范围内的所有体素及表索引） 默认false
  if(voxelmap_manager->config_setting_.map_sliding_en)
  {
    voxelmap_manager->mapSliding();
  }
  
  // 根据稠密地图使能决定使用 未体素化的去畸变点 还是 体素化后的去畸变点。并获得点数
  PointCloudXYZI::Ptr laserCloudFullRes(dense_map_en ? feats_undistort : feats_down_body);
  int size = laserCloudFullRes->points.size();
  // 根据上面选择的点云的点数，设置世界系下的点云指针
  PointCloudXYZI::Ptr laserCloudWorld(new PointCloudXYZI(size, 1));

  for (int i = 0; i < size; i++) 
  {
    // 循环每个点，其实就是将点云转到世界系下（没有 RGB，有反射强度）
    RGBpointBodyToWorld(&laserCloudFullRes->points[i], &laserCloudWorld->points[i]);
  }
  // 将世界系下的点云设为 待发布的世界系的点云
...
{
  // 首先将世界系点云置空并重置大小与载体系点云一样
  PointCloudXYZI().swap(*trans_cloud);
  trans_cloud->reserve(input_cloud->size());
  // 遍历每个载体系体素点,将体素点乘以外参和状态转到世界系。只包括了位置和反射强度信息
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  ...
  // }
  if (abs(last_timestamp_imu - stamp2Sec(msg->header.stamp)) > 1.0 && !imu_buffer.empty())  // Lidar与IMU的时间差超过1s了
  {
  ...
  // 将当前帧处理后的点云 推入 雷达原始数据缓存器 每帧都缓存
  lid_raw_data_buffer.push_back(ptr);
  // 将每帧点云的帧头时间都推入雷达帧头时间缓存器
  lid_header_time_buffer.push_back(cur_head_time);
  // 更新当前 lidar 帧头时间到最近的 lidar 时间戳
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  // 更新上个 IMU 时间戳，并推入 imu 消息到 imu 缓存器中。解开互斥锁
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  // sync for lidar(10Hz) and img(20Hz). 10Hz
  {
    static int frame_counter = 0;
    if (++frame_counter % img_down != 0) return;
  }
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      double img_capture_time = img_time_buffer.front() + exposure_time_init;  // 加上曝光时间
       ...
       * will be refresh. ***/
      // （第一帧（第一次运行时）last_lio_update_time 是个负数） 
      // 如果是第一次运行，那就是用第一帧点云的帧头时间更新 last_lio_update_time
      if (meas.last_lio_update_time < 0.0) meas.last_lio_update_time = lid_header_time_buffer.front();
      ...
      m.imu.clear();
      m.lio_time = img_capture_time;  // 测量组的 LIO 时间为图像捕获时间（获得图像后先进行 LIO 更新）
      mtx_buffer.lock();
      while (!imu_buffer.empty())
      {
        // 将最近 LIO 更新时间和图像捕获时间之间的 imu 数据推入到 测量组
      ...
      // 将下一个要处理的点云 给 当前处理点云 并 清空 下个处理的点云指针
      *(meas.pcl_proc_cur) = *(meas.pcl_proc_next);
      PointCloudXYZI().swap(*meas.pcl_proc_next);

      // 计算激光雷达缓存器中所有点云的总点数
      int lid_frame_num = lid_raw_data_buffer.size();
      int max_size = meas.pcl_proc_cur->size() + p_pre->max_points_num * lid_frame_num;
      // 将当前帧点云指针和下一帧点云指针的容量都扩大到所有点云点数
      ...
      {
        // 判断 如果出现激光雷达的帧头时间 晚于 图像捕获时间就退出循环
        if (lid_header_time_buffer.front() > img_capture_time) break;
        // 智能变量，获得激光雷达缓存器中头部点云，pcl。以及这个点云的帧头时间戳
        auto pcl(lid_raw_data_buffer.front()->points);
        double frame_header_time(lid_header_time_buffer.front());
        // 计算最大时间差上限 帧头时间 到 相机捕获时间的时间差 单位 ms
        ...
        for (int i = 0; i < pcl.size(); i++)
        {
          ...
          // 将该点时间偏差 加上 帧头时间与图像捕获时间的差 并存入激光雷达测量组 meas 的下个处理点云中
          {
      ...
      // 将测量组推入激光雷达测量组的 measures 并将 EKF 状态标志 更新为 LIO
      meas.measures.push_back(m);
    ...
    case LIO:
    {
      ...
      struct MeasureGroup m;
      m.vio_time = img_capture_time;                           // 设置测量组的 VIO 时间为 图像的捕获时间
      m.lio_time = meas.last_lio_update_time;                  // 设置测量组的 LIO 时间为 最近的 LIO 更新时刻
      m.img = img_buffer.front();                              // 将 img 缓存器的头部图像给测量组
      RCLCPP_INFO(this->node->get_logger(), "LIO img_buffer size: %ld", img_buffer.size());
      mtx_buffer.lock();
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  if (img_en)
  {
    ...
    *pcl_wait_pub += *pcl_w_wait_pub;
    // 这里简要介绍功能：将世界点转到相机系和像素系，只要该像素位于图像范围，对其进行色彩插值，并转为 ros 消息类型并发布
