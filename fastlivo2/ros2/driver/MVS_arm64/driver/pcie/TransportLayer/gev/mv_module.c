...
struct pci_device_id GigEDeviceId[] = {
    {GIGE_VENDOR_ID, GIGE_DEVICE_ID_GE1XXX, GIGE_VENDOR_ID, GIGE_SUBSYSTEM_ID_GE1XXX, 0, 0, 0},
    {GIGE_VENDOR_ID, GIGE_DEVICE_ID_GT100X, GIGE_VENDOR_ID, GIGE_SUBSYSTEM_ID_GT1002, 0, 0, 0},
    {GIGE_VENDOR_ID, GIGE_DEVICE_ID_GT100X, GIGE_VENDOR_ID, GIGE_SUBSYSTEM_ID_GT1004, 0, 0, 0},
    {GIGE_VENDOR_ID, GIGE_DEVICE_ID_GT110X, GIGE_VENDOR_ID, GIGE_SUBSYSTEM_ID_GT1102, 0, 0, 0},
    {GIGE_VENDOR_ID, GIGE_DEVICE_ID_GT110X, GIGE_VENDOR_ID, GIGE_SUBSYSTEM_ID_GT1104, 0, 0, 0},
    {GIGE_VENDOR_ID, GIGE_DEVICE_ID_GS100X, GIGE_VENDOR_ID, GIGE_SUBSYSTEM_ID_GS1004, 0, 0, 0},
    {0,}
};
...
struct pci_driver GigEDriver = {
    ...
    .probe = ModuleProbe,
    .remove = ModuleRemove,
    .suspend = ModuleSuspend,           // Device suspended
    .resume = ModuleResume,             // Device woken up
};
...
#else
	pstPciDevice->pstClass = class_create((const char *)chGigEClassName);
#endif
...
void MV_ReadIDPci(void* pPciDevice, unsigned int* nVendor, unsigned int* nDeviceID, unsigned int* nSubsystemVendorID, unsigned int* nSubsystemDeviceID)
{
    PPCI_DEVICE pstPciDevice = (PPCI_DEVICE) pPciDevice;
    *nVendor = pstPciDevice->pstPciDev->vendor;
    *nDeviceID = pstPciDevice->pstPciDev->device;
    *nSubsystemVendorID = pstPciDevice->pstPciDev->subsystem_vendor;
    *nSubsystemDeviceID = pstPciDevice->pstPciDev->subsystem_device;
}
...
void MV_FreeConsistent(void* pPciDevice, int nSize, void* pPhy, void* pVir)
{
    ...
    dma_free_coherent(&pstPciDevice->pstPciDev->dev, nSize, pVir, (unsigned long long)pPhy);
}
...
int ModuleProbe(struct pci_dev* pstPciDev, const struct pci_device_id* pstPciDeId)
{
    ...
    pstDeviceData->pBar0Phy = (void*)pci_resource_start(pstPciDev, 0);
    ...
    {
        pstDeviceData->pBar0Vir = ioremap((unsigned long long)pstDeviceData->pBar0Phy, pstDeviceData->nBar0Length);
    ...
    nRequestIrqNum = pci_alloc_irq_vectors(pstPciDev, 1, MSI_COUNT, PCI_IRQ_MSI);
...
{
    DEBUG_INFO(KERN_INFO"GigE Module Remove");
    size_t i = 0;
    ...
    MV_RemoveNIC(pstPciDevice, pstPciDevice->pDriverData);
...
int ModuleSuspend(struct pci_dev *dev, pm_message_t state)
{
    PPCI_DEVICE pstPciDevice = NULL;
    printk(KERN_INFO "Suspend enter , state[%d]", state.event);
    ...
    pstPciDevice = dev->dev.driver_data;
    if (NULL == pstPciDevice)
    {
        return -EINVAL;
    }

    return MV_ModuleSuspend(dev, pstPciDevice->pDriverData);
}

int ModuleResume(struct pci_dev *dev)
{
    PPCI_DEVICE pstPciDevice = NULL;
    printk(KERN_INFO "Resume enter!");
    ...
    return MV_ModuleResume(dev, pstPciDevice->pDriverData);
}
