...
@   返回值：本地时间
*******************************************************************************/
MV_CURRENT_TIME TM_GetCurrentTime(void)
{
    struct rtc_time tm         = { 0 };
    MV_CURRENT_TIME aCurrentTime = { 0 };
    ...
    aCurrentTime.tm_msec = txc.tv_nsec / 1000000;
    ...
    aCurrentTime.tm_msec = txc.time.tv_usec / 1000;
#endif
    tm.tm_year = tm.tm_year + 1900;
    tm.tm_mon = tm.tm_mon + 1;
    tm.tm_hour = (tm.tm_hour + 8) % 24;

    aCurrentTime.stRtcTm = *((MV_TM_RtcTime*)&tm);

    return aCurrentTime;
}


unsigned long long GigEGetTime(void)
{
    ...
    start_time = txc.tv_sec * 1000;
#else
    struct timeval start;
    do_gettimeofday(&start); 
    start_time = ((unsigned long long)start.tv_sec) * 1000;
#endif
    return start_time;
}

/*******************************************************************************
@   函数原型：unsigned long TM_GetTimestampMicrosec(void)
@   函数功能：获得系统当前时间的微秒数
@   参数： 无
@   返回值：系统当前时间的微秒数
*******************************************************************************/
unsigned long long TM_GetTimestampMicrosec(void)
{
    ...
    tv_sec = (unsigned long long)(txc.tv_sec);
    tv_nsec = (unsigned long long)(txc.tv_nsec);
    msce = txc.tv_sec * 1000000 + div_u64(tv_nsec, 1000);
#else
    struct  timeval   tv;
    unsigned long long tv_sec = 0;
