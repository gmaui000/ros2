...
{
    void*                   pUartPort;                          // uart_prot资源
    void*                   pCdvClass;
    void*                   pCdvClassDev;
    char                    chVirSerialDevClassName[20];
    char                    chVirSerialDevName[20];

    void*                   pPlatformData;                      // 平台设备数据，由采集卡驱动传过来的，platform_device -> platform_data
    void*                   pPciRegister;
    unsigned int            nPciRegisterLength;
    int                     nIrq;
    unsigned int            nIrqNum;
    void*                   pSerialDeviceContext;               // 单个虚拟串口设备上下文
    dev_t                   nDevMajor; 
    void*                   pPlatformDev;                       // 平台设备 platform_device
...
#else
extern void VirSerial_ModuleRemove(struct platform_device * pstPlatformDevice);
#endif
...
extern void virserial_remove_one_port(PSERIAL_DEVICE pstSerialDev);
